...
@auth
    ...
    <div class="fixed inset-y-0 left-0 w-64 bg-white shadow z-20">
        @include(auth()->user()->isAdmin() ? 'layouts.admin_sidebar' : 'layouts.user_sidebar')
    </div>

    <!-- Navbar -->
    <nav class="fixed top-0 left-64 right-0 h-16 bg-white shadow z-10">
        @include(auth()->user()->isAdmin() ? 'layouts.admin_navbar' : 'layouts.user_navbar')
    </nav>
@endauth
...
<main class="main-content">
    ...
    @if(session()->has('success'))
        <div class="bg-green-100 text-green-700 px-4 py-2 rounded m-4">
            {{ session('success') }}
        </div>
    @endif
