        ...
        <table class="min-w-full rounded-xl">
            <thead>
                <tr class="bg-gray-200">
                    ...
                    <th scope="col" class=" text-left text-sm leading-6 font-semibold text-gray-900 capitalize">Status</th>
            ...
            <tbody class="divide-y divide-gray-300">
                ...
                <td class=" p-1 whitespace-nowrap text-center text-sm leading-6 font-medium text-gray-900">{{ $loop->iteration }}</td>
                    ...
                    @php
                        ...
                        $firstLine = implode(' ', array_slice($words, 0, 8));
                        $secondLine = implode(' ', array_slice($words, 8));
                    @endphp
                    <td class=" p-1 whitespace-normal text-sm leading-6 font-medium text-gray-900">
                        {{ $firstLine }}@if($secondLine)<br>{{ $secondLine }}@endif
                    </td>

                    <td class=" p-1 whitespace-nowrap text-sm leading-6 font-medium text-left text-gray-900">{{ $offense->sanksi->keputusan_tindakan }}</td>
                    <td class=" p-1 whitespace-nowrap text-sm leading-6 font-medium text-left text-gray-900">
                    ...
                    <td class=" p-1 whitespace-nowrap text-sm leading-6 font-medium text-center text-gray-900">
                        <div class="flex items-center space-x-2">
                            <a href="{{ route('input-pelanggaran.show', $offense->id) }}" class="p-2  rounded-full  group transition-all duration-500  flex item-center">
                            ...
                            <a href=" {{ route(name: 'input-pelanggaran.edit', parameters:$offense) }}">
                                <button
                                    type="button"
                                    class="p-2 rounded-full group transition-all duration-500 flex item-center"
                                    @click="openModal = true; editingOffenseId = {{ $offense->id }}; currentStatus = '{{ $offense->status }}'"
                                    title="Edit Status"
                                >
                            ...
                            <form action="{{ route('input-pelanggaran.destroy', $offense) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus pelanggaran ini?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="p-2  rounded-full  group transition-all duration-500  flex item-center">
...
<div
    x-data="{ openModal: false, editingOffenseId: null, currentStatus: '' }"
    x-show="openModal"
    x-cloak
    ...
    >
        ...
        <form @submit.prevent="updateStatus">
            ...
            <div class="flex justify-end gap-2">
                <button type="button" @click="openModal = false" class="px-4 py-2 bg-gray-200 text-gray-700 rounded-md hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-gray-500">
                    Batal
                </button>
...
<script>
    // This script block handles the AJAX request for updating status
    function updateStatus() {
        const offenseId = this.editingOffenseId;
        const newStatus = this.currentStatus;

        fetch(`/input-pelanggaran/${offenseId}/update-status`, {
            method: 'PATCH',
            headers: {
        ...
        .then(data => {
            ...
            // This is where your line of code goes:
            const statusCell = document.querySelector(`tr[data-id="${offenseId}"] .text-center span`);
            if (statusCell) {
            ...
            alert('Status updated successfully!'); // Or use a more sophisticated notification
            location.reload(); // Reload the page to reflect changes properly (can be optimized with live updates)
        })
        .catch(error => {
