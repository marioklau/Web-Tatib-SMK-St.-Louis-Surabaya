        ...
        <table class="min-w-full rounded-xl">
            ...
            <tbody class="divide-y divide-gray-300">
                ...
                <td class="p-1 whitespace-nowrap text-center text-sm leading-6 font-medium text-gray-900">{{ $loop->iteration }}</td>
                    <td class="p-1 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ $offense->siswa->nama_siswa }}</td>
                    <td class="p-1 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ $offense->siswa->kelas->nama_kelas ?? '-' }}</td>
                    @php
                    ...
                    <td class="p-1 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{{ $offense->sanksi->nama_sanksi }}</td>
                    <td class="p-1 whitespace-nowrap text-sm leading-6 font-medium text-center text-gray-900">
                        {{ $offense->status ?? 'Not Done' }}
                    </td>
                    <td class="p-1 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">
                        <div class="flex items-center space-x-2">
                            <!-- Tombol Toggle Status -->
                            <!-- <form action="{{ route('input-pelanggaran.update-status', $offense->id) }}" method="POST" onsubmit="return confirm('Ubah status pelanggaran ini?');">
                                @csrf
                                @method('PATCH')
                                <input type="hidden" name="status" value="{{ $offense->status === 'Done' ? 'Not Done' : 'Done' }}">
